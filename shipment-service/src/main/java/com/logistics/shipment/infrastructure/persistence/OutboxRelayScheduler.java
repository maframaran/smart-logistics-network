...
// row to await broker acknowledgment before marking it published — this is the one place
// in the codebase that's allowed to wait synchronously on a Kafka send, since the whole
// point is to know whether the publish actually succeeded before declaring the row done.
// Lives in infrastructure/persistence (not infrastructure/messaging) so it can access the
...
class OutboxRelayScheduler {
    ...
    private static final Map<String, Class<? extends DomainEvent>> EVENT_TYPES = Map.of(
            ...
            "ShipmentAssigned", ShipmentAssigned.class,
            "ShipmentCancelled", ShipmentCancelled.class
    );
    ...
    OutboxRelayScheduler(OutboxJpaRepositoryPort outboxJpa, ShipmentEventPublisher publisher, ObjectMapper objectMapper) {
